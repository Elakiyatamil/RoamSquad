...
const FoodForm = ({ food, destinationId, onClose }) => {
    ...
    const mutation = useMutation({
        mutationFn: async (data) => {
            ...
            return await apiClient.post(`/destinations/${destinationId}/food`, data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries(['food', destinationId]);
            onClose();
        },
...
const FoodOptionsManager = () => {
    ...
    const { data: foodOptions = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await apiClient.get(`/destinations/${selectedDestId}/food`);
            return res.data;
        },
