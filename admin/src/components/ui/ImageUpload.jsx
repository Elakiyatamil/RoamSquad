            ...
            <div className="relative group">
                {preview ? (
                    <div className="relative w-full h-48 rounded-2xl overflow-hidden border border-ink/5 bg-white shadow-sm">
                        {preview && (preview.includes('video') || preview.match(/\.(mp4|webm|ogg)$/)) ? (
                            <video src={getImgUrl(preview)} className="w-full h-full object-cover" muted loop autoPlay />
                        ) : (
                            <img src={getImgUrl(preview)} alt="Preview" className="w-full h-full object-cover" />
                        )}
